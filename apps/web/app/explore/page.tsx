  ...
  return (
    <DossierPageShell tickerItems={TICKER_ITEMS}>
      <div className="px-6 py-10 md:px-10">
        <p className="dossier-kicker text-[var(--color-ink-faded)]">DECLASSIFIED INDEX</p>
        ...
        <p className="mt-3 max-w-xl dossier-body">
          Operative-submitted intelligence forms cleared for public access. Unlisted transmissions
          require a direct handler link.
        </p>

        {isPending ? (
          <div className="mt-12 flex items-center gap-3">
            <Spinner className="size-5" />
            <span className="dossier-meta text-[var(--color-ink-faded)]">INDEXING RECORDS...</span>
          </div>
        ) : !forms?.length ? (
          <div className="mt-12 border-2 border-dashed border-[var(--color-ink-faded)] py-16 text-center">
            ...
            <p className="mt-6 dossier-body">No public dossiers have been published yet.</p>
          </div>
        ) : (
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {forms.map((form) => (
              ...
              >
                ...
                {form.description ? (
                  <p className="mt-2 line-clamp-2 dossier-body text-sm">{form.description}</p>
                ) : null}
                <p className="mt-4 dossier-label text-[var(--color-brass)]">OPEN TERMINAL →</p>
              </Link>
            ))}
          </div>
        )}
