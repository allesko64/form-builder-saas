"use client";

import { signIn } from "@repo/auth/client";
import { useState } from "react";
import { toast } from "sonner";

type GoogleSignInButtonProps = {
  /** Path or full URL to return to after OAuth (e.g. "/dashboard"). */
  callbackURL?: string;
};

function GoogleIcon() {
  return (
    <svg
      aria-hidden
      className="size-4 shrink-0"
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="var(--color-ink)"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="var(--color-ink-faded)"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="var(--color-ink-faded)"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="var(--color-ink)"
      />
    </svg>
  );
}

/** OAuth must redirect back to the Next.js app, not the API origin. */
function resolveCallbackURL(path: string): string {
  if (path.startsWith("http://") || path.startsWith("https://")) {
    return path;
  }
  const normalized = path.startsWith("/") ? path : `/${path}`;
  if (typeof window !== "undefined") {
    return `${window.location.origin}${normalized}`;
  }
  const webOrigin = process.env.NEXT_PUBLIC_WEB_APP_URL ?? "http://localhost:3000";
  return `${webOrigin}${normalized}`;
}

export function GoogleSignInButton({ callbackURL = "/dashboard" }: GoogleSignInButtonProps) {
  const [isLoading, setIsLoading] = useState(false);

  async function handleGoogleSignIn() {
    setIsLoading(true);

    try {
      const { error } = await signIn.social({
        provider: "google",
        callbackURL: resolveCallbackURL(callbackURL),
      });

      if (error) {
        toast.error(error.message ?? "Could not sign in with Google");
        return;
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : "Could not reach the auth server";
      const hint =
        message.includes("fetch") || message.includes("Failed")
          ? " — is the API running on port 8000?"
          : "";
      toast.error(`${message}${hint}`);
      console.error("Google sign-in failed:", err);
    } finally {
      setIsLoading(false);
    }
  }

  return (
    <button
      type="button"
      disabled={isLoading}
      onClick={handleGoogleSignIn}
      className="flex w-full items-center justify-center gap-2 rounded-none border border-[var(--color-ink-faded)] bg-[var(--color-paper)] px-4 py-[14px] dossier-meta text-[var(--color-ink)] transition-colors hover:bg-[var(--color-paper-dark)] disabled:opacity-70"
    >
      <GoogleIcon />
      {isLoading ? "REDIRECTING..." : "AUTHENTICATE VIA EXTERNAL CHANNEL"}
    </button>
  );
}
